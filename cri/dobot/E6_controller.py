...
import numpy as np
import ipdb
from cri.dobot.E6_client import E6Client
from cri.controller import RobotController


class E6Controller(RobotController):
    ...
    def get_robot_mode(self):
        return self._client.get_robot_mode()
    @property
    def servo_mode(self):
    ...
    def servo_move_linear(self, pose, elbow=None):
        ...
        if elbow is not None:
            warnings.warn("elbow property not implemented in Dobot CR Controller")
        self._commanded_pose = pose
        self._client.servo_move_linear(pose)
    def rel_move_linear(self, pose):
        ...
        self._client.rel_move_linear(pose)
