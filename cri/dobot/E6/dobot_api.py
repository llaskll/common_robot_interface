...
class DobotApiDashboard(DobotApi):
    ...
    def GetAngle(self):
        ...
        string = "GetAngle()"
        array = re.search(r'\{([^}]+)\}', self.sendRecvMsg(string))
        content = array.group(1).replace(' ', '')
    ...
    def MovL(self, a1, b1, c1, d1, e1, f1, coordinateMode, user=-1, tool=-1, a=-1, v=-1, speed=-1, cp=-1, r=2):
        """
        描述
        从当前位置以直线运动⽅式运动⾄⽬标点。
        必选参数
    ...
    def ServoP(self, X, Y, Z, RX, RY, RZ, t=-1.0, aheadtime=-1.0, gain=-1.0):
        ...
        # ipdb.set_trace()
        self.send_data(string)
        # 'MovLIO(pose={313.191711,-25.314899,252.468994,180.000000,0.000000,-150.000000},{0,100,3,1})'
        return self.wait_reply()
                ...
                with self.__globalLockValue:
                    if self.__feedData.robotEnableStatus:
                        if self.__feedData.robotCurrentCommandID > p2Id:
                            print('robotCurrentCommandID', self.__feedData.robotCurrentCommandID)
